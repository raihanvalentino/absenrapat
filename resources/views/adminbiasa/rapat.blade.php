...
<body class="bg-gray-100 min-h-screen font-sans antialiased">
    ...
    <div class="p-6 sm:ml-64 mt-20">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between mb-6 gap-4">
            <h2 class="text-2xl font-bold text-gray-800">Daftar Rapat</h2>
                ...
                <div class="p-6">
                    <form action="{{ route('storeTambahRapat') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                <label class="block mb-2 text-sm font-medium text-gray-900">Upload Notulensi</label>
                                <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50" name="notulensi" type="file" accept=".pdf,.doc,.docx">
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label class="block mb-2 text-sm font-medium text-gray-900">Link Materi</label>
                                    <input type="url" name="materi" placeholder="https://..." class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5">
                                </div>
                                <div>
                                    <label class="block mb-2 text-sm font-medium text-gray-900">Link Dokumentasi</label>
                                    <input type="url" name="dokumentasi" placeholder="https://..." class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5">
                                </div>
                            </div>
                        </div>
    ...
    @foreach($rapat as $item)
                    ...
                    <div class="p-6">
                        <form method="POST" action="{{ route('updateRapat', ['id' => $item->rapat_id]) }}" enctype="multipart/form-data">
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    <label class="block mb-2 text-sm font-medium text-gray-900">Update Notulensi</label>
                                    <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-white" name="notulensi" type="file" accept=".pdf,.doc,.docx">
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="url" name="materi" value="{{ $item->materi }}" class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5">
                                    </div>
                                    <div>
                                        <label class="block mb-2 text-sm font-medium">Link Dokumentasi</label>
                                        <input type="url" name="dokumentasi" value="{{ $item->dokumentasi }}" class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5">
                                    </div>
                                </div>
                                <div>
                                    <label class="block mb-2 text-sm font-medium text-gray-900">Status Presensi</label>
                                    <select name="status_presensi" class="bg-gray-50 border border-gray-300 rounded-lg w-full p-2.5">
                                        <option value="auto" {{ $item->status_presensi == 'auto' ? 'selected' : '' }}>Otomatis</option>
                                        <option value="buka" {{ $item->status_presensi == 'buka' ? 'selected' : '' }}>Buka</option>
                                        <option value="tutup" {{ $item->status_presensi == 'tutup' ? 'selected' : '' }}>Tutup</option>
                                    </select>
                                </div>
                            </div>
                            <div class="mt-6 flex justify-end">
                                <button type="submit" class="text-white bg-[#153D53] hover:bg-[#102e3f] font-medium rounded-lg text-sm px-5 py-2.5">Update</button>
                            </div>
                        </form>
                    </div>
                </div>
        ...
        <div id="delete-modal-{{ $item->rapat_id}}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed inset-0 z-50 flex justify-center items-center w-full h-full bg-black/50 backdrop-blur-sm">
            <div class="relative p-4 w-full max-md max-h-full">
                <div class="relative bg-white rounded-lg shadow-xl text-center p-6">
                    <h3 class="mb-5 text-lg font-normal text-gray-500">Hapus rapat ini?</h3>
        ...
        <div id="peserta-modal-{{ $item->rapat_id }}" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed inset-0 z-50 flex justify-center items-center w-full h-full bg-black/30 backdrop-blur-sm">
            <div class="relative p-4 w-full max-w-7xl max-h-full">
                <div class="relative bg-white rounded-xl shadow-2xl overflow-hidden">
                    <div class="flex items-start justify-between p-5 border-b bg-gray-50">
                        <div>
                            <h3 class="text-xl font-bold text-gray-800">Daftar Peserta: <span class="text-[#153D53]">{{ $item->nama_rapat }}</span></h3>
                            <div class="mt-2 text-sm text-gray-500">{{ \Carbon\Carbon::parse($item->tanggal)->format('d F Y') }}</div>
                        </div>
                    ...
                    <div class="p-6 space-y-6">
                        <div class="flex flex-col md:flex-row justify-between items-center gap-4 bg-blue-50 border p-4 rounded-lg">
                            ...
                            <div class="flex gap-2">
                                {{-- Tombol Excel (Route Baru) --}}
                                <a href="{{ route('admin.exportPeserta', $item->rapat_id) }}" 
                                   class="flex items-center px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 transition">
                                   Excel
                                </a>
                                {{-- Tombol Print (Parameter Lengkap) --}}
                                <button onclick="printTable(
                                    'table-peserta-{{ $item->rapat_id }}', 
                                    '{{ $item->nama_rapat }}', 
                                    '{{ $item->nama_rapat }}', 
                                    '{{ \Carbon\Carbon::parse($item->tanggal)->locale('id')->isoFormat('dddd, D MMMM Y') }}', 
                                    '{{ $item->ruang }}', 
                                    '{{ $item->waktu_mulai }} - {{ $item->waktu_selesai }}'
                                )" class="flex items-center px-4 py-2 text-sm font-medium text-white bg-gray-700 rounded-lg hover:bg-gray-800 transition">
                        ...
                        <div class="relative overflow-hidden border rounded-lg shadow-sm">
                            @if($item->peserta->count() > 0)
                                ...
                                <table id="table-peserta-{{ $item->rapat_id }}" class="w-full text-sm text-left text-gray-600">
                                    <thead class="text-xs text-gray-700 uppercase bg-gray-100 sticky top-0">
                                        <tr>
                                            ...
                                            <th class="px-4 py-4 whitespace-nowrap">NAMA</th>
                                            <th class="px-4 py-4 whitespace-nowrap">NIP/NIK</th>
                                            <th class="px-4 py-4 whitespace-nowrap">PANGKAT/GOL</th>
                                            <th class="px-4 py-4 whitespace-nowrap">JENIS</th>
                                            <th class="px-4 py-4 whitespace-nowrap">JABATAN</th>
                                            <th class="px-4 py-4 whitespace-nowrap">UNIT KERJA</th>
                                            <th class="px-4 py-4 whitespace-nowrap">ASAL INSTANSI</th>
                                            <th class="px-4 py-4 whitespace-nowrap">NO. KONTAK</th>
                                    ...
                                    <tbody class="divide-y divide-gray-200 bg-white">
                                        ...
                                        <tr class="hover:bg-blue-50 transition">
                                            ...
                                            <td class="px-4 py-4 font-semibold text-gray-900 whitespace-nowrap">{{ $peserta->nama }}</td>
                                            <td class="px-4 py-4 whitespace-nowrap">{{ $peserta->nip_nik ?? '-' }}</td>
                                            <td class="px-4 py-4 whitespace-nowrap">{{ $peserta->pangkat_golongan ?? '-' }}</td>
                                            <td class="px-4 py-4 whitespace-nowrap">
                                                <span class="px-2 py-0.5 rounded text-xs {{ $peserta->jenis_peserta == 'Internal' ? 'bg-blue-100 text-blue-800' : 'bg-orange-100 text-orange-800' }}">{{ $peserta->jenis_peserta }}</span>
                                            </td>
                                            <td class="px-4 py-4 whitespace-nowrap"><span class="bg-gray-100 text-gray-800 text-xs px-2 py-0.5 rounded">{{ $peserta->jabatan ?? '-' }}</span></td>
                                            <td class="px-4 py-4 whitespace-nowrap">{{ $peserta->unit_kerja ?? '-' }}</td>
                                            <td class="px-4 py-4 whitespace-nowrap">{{ $peserta->asal_instansi ?? '-' }}</td>
                                            <td class="px-4 py-4 whitespace-nowrap font-mono text-xs">{{ $peserta->nomor_kontak ?? '-' }}</td>
                                            <td class="px-4 py-4 text-center no-print">
                                                <form action="{{ route('deletePeserta', $peserta->peserta_id) }}" method="POST" onsubmit="return confirm('Hapus peserta?');">
                                                    @csrf @method('DELETE')
                    ...
                    <div class="flex items-center justify-end p-5 border-t bg-gray-50">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            searchInput.addEventListener('keyup', () => { clearTimeout(typingTimer); typingTimer = setTimeout(submitForm, 500); });
            filterDate.addEventListener('change', submitForm);
        });

        function printTable(tableId, title, agenda, tanggal, tempat, waktu) {
            // 1. Ambil tabel asli dan bersihkan kolom aksi (tombol hapus)
            var table = document.getElementById(tableId).cloneNode(true);
            var noPrintElements = table.querySelectorAll('.no-print');
            noPrintElements.forEach(el => el.remove());

            // 2. Buka jendela print baru
            var win = window.open('', '', 'height=800,width=1200');
            
            // 3. Masukkan Struktur HTML dan CSS
            win.document.write(`
                <html>
                <head>
                    <title>Print Daftar Hadir</title>
                    <style>
                        @page { size: A4 landscape; margin: 1.5cm; }
                        body { font-family: "Arial", sans-serif; color: #000; margin: 0; padding: 0; }
                        .header { text-align: center; margin-bottom: 25px; }
                        .header h2 { margin: 0; text-transform: uppercase; font-size: 22px; text-decoration: underline; font-weight: bold; }
                        
                        .info-rapat { width: 100%; margin-bottom: 15px; font-size: 14px; font-weight: bold; border-collapse: collapse; }
                        .info-rapat td { padding: 4px 0; vertical-align: top; border: none !important; }
                        ...
                        table.main-table th { background-color: #f2f2f2 !important; text-align: center; font-weight: bold; -webkit-print-color-adjust: exact; }
                ...
                <body>
                    ...
                    <table class="info-rapat">
                        <tr><td width="150">Agenda</td><td>: ${agenda}</td></tr>
            ...
            // 4. Masukkan tabel data ke dalam container
            win.document.getElementById('table-container').appendChild(table);
            table.classList.add('main-table');

            // 5. Eksekusi Print
            win.document.close(); // Penting untuk menutup aliran dokumen
            
            // Gunakan onload agar dipastikan konten sudah ter-render
            win.onload = function() {
                win.focus();
                win.print();
                win.close();
            ...
            // Backup jika onload tidak jalan di browser tertentu
            setTimeout(function() {
                if(!win.closed) {
                    win.print();
                    win.close();
